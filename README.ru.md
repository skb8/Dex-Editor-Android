...
- **Инструменты для LLM агента (Tools)**:
  ...
  - `dex_search` — Быстрый поиск в пуле по классам, методам, полям, строкам или коду; поддерживает обычный и regex-режим (`regex`, `ignoreCase`, `limit`).
  ...
  - `dex_validate` — Проверка pending/переданного Smali без сохранения файлов.
  - `dex_diff` — Предпросмотр line-diff перед применением/сохранением изменений.
  - `dex_export_smali` / `dex_import_smali` — Экспорт и импорт классов как `.smali` файлов или текста.
  - `dex_rename_class`, `dex_rename_method`, `dex_rename_field` — Переименование descriptor/reference с валидацией.
  - `dex_get_call_graph` — Просмотр входящих/исходящих вызовов методов.
  - `dex_get_constants` — Извлечение строковых/числовых констант из Smali.
  - `dex_patch_batch` — Последовательное применение нескольких MCP операций одним запросом.
...
- **Двусторонняя синхронизация**:
  ...
  - Если пользователь сам выбирает один или несколько DEX на главном экране приложения (встроенный проводник записывает несколько путей через `;`), и MCP сервер при этом запущен, файлы автоматически загружаются в память сервера как multi-dex.
